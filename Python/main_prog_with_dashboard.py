...
def minimax(board, depth, alpha, beta, maximizingPlayer):
	...
	if maximizingPlayer:
		...
		for col in valid_locations:
			...
			recursionCount += 1
			# print("recursion count is", recursionCount)
...
def play_game(depth, logThinkTimes=True, displayDashboard = True):
	...
	while (no_winner):
		if (turn == PLAYER):
			...
			# these lines don't work properly...
			# if np.any(oldBoard == None): # if `interpretBoard` returns None, then some/all of the four boundary codes were not found in frame
				...
				# while np.any(oldBoard == None): 
				# oldBoard = interpretBoard(takePictureOfBoard())
			
	  # this should be uncommented for turn-taking autonomy, but is currently disabled for org fair
...
#            input()
			...
			# Minimax and drop the piece, making sure to pass in upside-down board!
			global recursionCount
			recursionCount = 15 # reset to zero before running minimax
